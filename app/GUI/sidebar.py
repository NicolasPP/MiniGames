...
class Sidebar:
	...
	def add_sidebar_content(self):
		...
		quit = Button((PADDING, self.height - PADDING - BUTTON_H), (BUTTON_W - PADDING) // 2, BUTTON_H, "Black", on_click = quit_game, lable = "Quit", offset = self.topleft, show_lable= False)
		full_screen = Button(((PADDING * 2) + (BUTTON_W - PADDING) // 2, self.height - PADDING - BUTTON_H), (BUTTON_W - PADDING) // 2, BUTTON_H, "Green", on_click = fullscreen, lable = "Fullscreen", offset = self.topleft, show_lable= False, button_type = Button_Type.SWITCH)
		quit.style(style_quit, quit)
		full_screen.set_active_style(style_quit, full_screen)
		full_screen.set_inctive_style(test_style2, full_screen)
